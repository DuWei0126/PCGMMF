...
parser.add_argument('--task', type=str, choices=['Recurrence_metastasis_vs_normal', 'Dead_vs_Alive'])
#更改训练集：测试集=7：3
...
if args.task == 'Recurrence_metastasis_vs_normal':
    args.n_classes=2
    # 标签地址
    dataset = Generic_WSI_Classification_Dataset(csv_path ='Step_4.csv',
                                                 shuffle = False,
                                                 seed = args.seed,
                                                 print_info = True,
                                                 label_dict = {'0':0, '1':1},
...
"""if args.task == 'Dead_vs_Alive':
    args.n_classes=2
    # 标签地址
    dataset = Generic_WSI_Classification_Dataset(csv_path = './SC-Step_4_207.csv',
                            shuffle = False,
                            seed = args.seed,
                            print_info = True,
                            label_dict = {'0':0, '1':1},
...
if __name__ == '__main__':
    ...
    # 分割保存地址
        ...
        print(args.k, test_num, lf)
        dataset.create_splits(k = args.k, test_num = test_num, label_frac=lf)
        for i in range(args.k):
            dataset.set_splits()
            descriptor_df = dataset.test_split_gen(return_descriptor=True)
            train_split, test_split, train_index, test_index = dataset.return_splits(from_id=True)
