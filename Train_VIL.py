...
#from models.resnet import resnet50_baseline, resnet18_baseline
import argparse
from utils.utils_tt import print_network, collate_features
...
if __name__ == '__main__':
    ...
    """model = torch.hub.load(
        "nx-ai/vision-lstm",
        "VisionLSTM",
        dim=384,  # latent dimension (192 for ViL-T)
        depth=24,  # how many ViL blocks
        patch_size=16,  # patch_size (results_5 in 196 patches for 224x224 images)
        input_shape=(3, 512, 512),  # RGB images with resolution 224x224
        output_shape=(1024,),  # classifier with 1000 classes
        drop_path_rate=0.05,  # stochastic depth parameter
        stride=None,  # set to 8 for long-sequence fine-tuning
    )"""

    model = torch.hub.load("nx-ai/vision-lstm", "vil-base")
    #model.torch.load('./weights/vil_base16_e400_in1k.th')
