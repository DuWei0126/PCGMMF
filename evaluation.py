...
def test_model(model, test_loader, n_classes, device=None):
    ...
    for batch_idx, (data, label, case_id) in enumerate(test_loader):
        ...
        probs = Y_prob.cpu().numpy()    #预测出的概率
        hats = Y_hat.cpu().numpy()  #预测出的标签
        all_probs[batch_idx] = probs
        all_hats[batch_idx] = hats
        all_labels[batch_idx] = label.item()    #真实标签
...
if __name__ == "__main__":
    dataset.load_from_h5(True)  #Start. h5 file related content
    #Obtain the data and index of the test set
    ...
    #Specify the model and load the parameters of the trained model
    model = TN_TOAD(**model_dict)
    model.load_state_dict(torch.load("./results_5/dataset_1_result_s5/s_1_checkpoint.pt"))  #Specify the parameter file of the model here
    #Initialize the partitioned test set as a dataloader object
    test_split = datasets
