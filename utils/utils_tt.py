...
def collate_S(batch):
	...
	# gender = torch.LongTensor([item[2] for item in batch])
...
def collate_MIL(batch):
	img = torch.cat([item[0] for item in batch], dim = 0)
	label = torch.LongTensor([item[1] for item in batch])
	case_id = [item[2] for item in batch]
	# gender = torch.LongTensor([item[2] for item in batch])
	# return [img, label, gender...]
...
def get_simple_loader(dataset, batch_size=1, num_workers=0):
	...
	loader = DataLoader(dataset, batch_size=batch_size, sampler = sampler.SequentialSampler(dataset), collate_fn = collate_S, **kwargs)
	return loader 
...
def generate_split(cls_ids, test_num, samples, n_splits = 5,
